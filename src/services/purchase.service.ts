...
import { purchases, purchaseItems } from '../db/schema/purchases.js';
...
export async function createPurchase(tenantId: string, userId: string, input: CreatePurchaseInput) {
  return db.transaction(async (tx) => {
    ...
    const [tenant] = await tx.select({ gstScheme: tenants.gstScheme })
    ...
    // 5. Create supplier ledger entry (debit = amount owed)
    ...
    // 6. Update supplier outstanding balance
...
export async function listPurchases(tenantId: string, filters: {
  ...
  const items = await db.select().from(purchases)
